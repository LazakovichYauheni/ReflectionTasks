    ...
    public static class CommonTasks
    {
        ...
        public static IEnumerable<string> GetPublicObsoleteClasses(string assemblyName) {
            ...
            return assembly.GetTypes()
                           .Where(x => x.IsClass && x.IsPublic && x.GetCustomAttributes(typeof(ObsoleteAttribute), true)
                           .Any()).Select(x => x.Name);
        }
        ...
        public static void SetPropertyValue(this object obj, string propertyPath, object value)
        {
            ...
            if (prop.Length > 1)
                obj = obj.GetPropertyValue<object>(string.Join(".", prop.Take(prop.Length-1)));

                obj.GetType().BaseType.GetProperty(prop.Last()).GetSetMethod(true).Invoke(obj,new object[] { value});
